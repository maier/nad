...
* OpenBSD - manual service configuration/installation required by user
  ...
  >fi
  >```
  > Will start NAD and redirect logging to syslog via the `logger` command. If you would rather redirect logging to a file replace the `--syslog` option with your redirection.
