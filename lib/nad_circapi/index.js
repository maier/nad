...
const lookup_hostname = require('nad_os_util').lookup_hostname;
...
function post_graph(graph) {
    client.post(
        ...
        (code, error, body) => {
            if (code !== 200) {
                console.error(`Could not create graph ${graph.title}`, error, body);
            }
        }
    );
}
            ...
            if (code === 401) {
                ...
                if (needs_update) {
                    client.put(
                        ...
                        (_code, _error, _body) => {
                            if (code !== 200) {
                                console.error(`Error from Circonus API: ${_error}`);
                                process.exit(1);
                            }
