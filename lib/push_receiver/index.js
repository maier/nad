...
function __aggr(tgt, src) {
    for (const key in src) {
        ...
        if (Array.isArray(src[key]._value)) {
            tgt[key]._value = tgt[key].concat(src[key]._value);
        } else {
...
module.exports = class PushReceiver {
    constructor(options) {
        this.set = {};
        this.log = options.log.child({ module: 'push_receiver' });
    }
    ...
    store_incoming_data(name, incoming_data) {
        this.log.debug({ name, raw_data: incoming_data.toString() }, 'received data');
        ...
        try {
            ...
            this.log.error({ err: err.message }, 'parsing incoming data');
            return;
        }

        this.log.trace({ name, parsed_data: data, location_idx: this_idx % 60 }, 'storing data');
