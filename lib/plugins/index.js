...
class Plugins {
    ...
    scan(cb) {
        ...
        function genStatCallback(name, file) {
            return (err, sb) => {
                ...
                if (!(name in scripts)) {
                    scripts[name] = Object.assign({}, defaultPlugin);
                }
                ...
                fs.access(cfgFile, fs.R_OK, (accessErr) => {
                    ...
                    // NOTE: _not_ a require() here, requires are cached and ignore changes
    ...
    _exec_plugin(plugin, cb, req, args, pluginInstance) {
        this.log.info({ name: pluginInstance }, 'executing plugin');
        ...
        if (plugin.is_native) {
            plugin.native_obj.run(plugin,
                (_plugin, _metrics, _pluginInstance) => {
                    past_results[_pluginInstance] = _metrics;
                    cb(_plugin, _metrics, _pluginInstance);
                }, req, args, pluginInstance);
            return;
        }
        ...
        // the output
        function handle_output(_plugin, _cb, _pluginInstance) {
            ...
            past_results[_pluginInstance] = results;

            // execute the callback
            _cb(_plugin, results, _pluginInstance);
        }
    ...
    _run_plugin(plugin, req, cb) {
        if (plugin.running) {
            this.log.debug({ name: plugin.name }, 'plugin already running');
        ...
        for (const pluginInstance in plugin.config) {
            if ({}.hasOwnProperty.call(plugin.config, pluginInstance)) {
                this._exec_plugin(plugin, cb, req, plugin.config[pluginInstance], `${plugin.name}\`${pluginInstance}`);
            }
        }
    }
    ...
    run(req, res, which) {
        ...
        if (which !== null) {
            req.nad_run_count = 1;
            this._run_plugin(scripts[which], req, (def, results, pluginInstance) => {
                req.nad_run_count--;
                metrics[pluginInstance] = results;
                send_complete();
            });
            return;
        }
        ...
        // run all plugins
        for (const pluginID in scripts) {
            if ({}.hasOwnProperty.call(scripts, pluginID)) {
                this._run_plugin(scripts[pluginID], req, (def, results, pluginInstance) => {
                    req.nad_run_count--;
                    metrics[pluginInstance] = results;
                    send_complete();
                });
        ...
        if (this.pushReceiver !== null) {
            ...
            this._run_plugin(this.pushReceiver, req, (def, results, pluginInstance) => {
                req.nad_run_count--;
                if (results !== null) {
                    ...
                    if (self.debug_dir !== null) {
                        dutil.write_debug_output(pluginInstance, [ 'Returning push_receiver data', JSON.stringify(results) ], self.debug_dir, self.wipe_debug_dir);
                    }
                }
                send_complete();
            });
