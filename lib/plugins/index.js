...
class Scanner {
    ...
    * read_plugin_dir() {
        const entries = fs.readdirSync(this.plugin_dir);

        for (const entry of entries) {
            const file_parts = path.parse(path.join(this.plugin_dir, entry));

            if (file_parts.name === '' || file_parts.ext === '') {
                this.log.warn({ file: file_parts.base }, 'ignoring, invalid name format');
                continue;
            }

            if (file_parts.ext === '.conf' || file_parts.ext === '.json') {
                this.log.warn({ file: file_parts.base }, 'ignoring, config');
                continue;
            }
    ...
    //         if (file_parts.name === '' || file_parts.ext === '') {
    //             self.log.warn({ file: file_parts.base }, 'ignoring, invalid name format');
    //             return new_list;
    //         }

    //         if (file_parts.ext === '.conf' || file_parts.ext === '.json') {
    //             self.log.warn({ file: file_parts.base }, 'ignoring, config');
    //             return new_list;
    //         }
