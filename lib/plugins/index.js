...
class Scanner {
    ...
    scan(cb) {
        ...
        this.scan_dir(this.plugin_dir).
            ...
            then(() => {
                ...
                const msg = 'scanning plugin directory';

                self.log.fatal({ err: err.message, dir: self.plugin_dir }, msg);
                console.error(self.pfx_error, self.plugin_dir, msg, err);
                process.exit(1);
            }).
            then(() => {
                ...
                const msg = 'scan callback';

                self.log.fatal({ err: err.message }, msg);
                console.error(self.pfx_error, msg, err);
                process.exit(1);
            });
    }
    ...
    // filter eliminates directory entries from scan_dir which are invalid (based on pathname)
    // it returns a list of verification promises
    filter(files) {
        const self = this;
        ...
        function reducer(new_list, file_name) {
            ...
            if (file_parts.name === '' || file_parts.ext === '') {
                self.log.warn({ file: file_parts.name }, 'ignoring, invalid name format');
                return new_list;
            }
    ...
    // _verifier stats the file and performs more thorough verification
    ...
    // verify resolves all verifier promises from filter
    verify(list) {
            ...
            for (const plugin_id in plugin_list) {
                if ({}.hasOwnProperty.call(plugin_list, plugin_id)) {
                    if (plugin_list[plugin_id].generation < self.generation) {
                        self.log.debug({ id: plugin_id }, 'removing expired/invalid plugin');
                        if (this.file_watch) {
...
class Plugins {
    constructor(push_receiver) {
        ...
        this.log = settings.logger.child({ module: 'plugins' });
    ...
    _run_plugin(plugin, req, cb) {
        if (plugin.running) {
            this.log.debug({ id: plugin.id }, 'plugin already running');
        ...
        this.log.debug({ id: plugin.id, config: plugin.config }, 'applying plugin config');
