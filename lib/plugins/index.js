...
const default_plugin = {
    ...
    stat: null, // fs.Stat from last scan
            ...
            for (const plugin_status of list) {
                ...
                if (def.is_native) {
                    ...
                    if (!def.native_obj.run) {
                        self.log.error({ id: def.name }, 'native plugin missing "run" method, removing plugin');
                        def.generation = -1;
                        continue;
                    }
                }
...
class Plugins {
    ...
    inventory(full) {
        ...
        for (const plugin_id in plugin_list) {
            if ({}.hasOwnProperty.call(plugin_list, plugin_id)) {
                inventory[plugin_id] = {
                    name: plugin_list[plugin_id].name,
