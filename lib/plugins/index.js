...
// generic plugin object, used as basis for all plugins (including push_receiver)
const default_plugin = {
    id: null,         // id (name) of the plugin
            ...
            for (const plugin_status of list) {
                ...
                if (def.is_native) {
                    ...
                    try {
                        ...
                        self.log.error({ err: perr, file: def.command }, 'unable to load native plugin code, removing plugin');
                        def.generation = -1;
                        continue;
                    }
                    ...
                    if (!def.native_obj.run) {
                        self.log.error({ id: def.id }, 'native plugin missing "run" method, removing plugin');
                        def.generation = -1;
                        continue;
                    }
                }
                ...
                if (def.config !== null) {
                    if (Object.keys(def.config).length === 0) {
                        self.log.error({ cfg_file: def.config_file }, 'invalid config, no instances defined, removing plugin');
                        def.config = null;
                        def.generation = -1;
                        continue;
                    }
...
module.exports.default_plugin = default_plugin;
