...
const log = settings.statsd.logger.child({ submodule: 'statsd' });
...
function getFlushTimeout(interval) {
    let timer_interval = interval - ((Date.now() - settings.statsd.start_time) % settings.statsd.flushInterval);

    if (timer_interval < 0) {
        log.warn(`calculated negative flush timer_interval (${timer_interval}), resetting to ${settings.statsd.flushInterval}...`);
        timer_interval = settings.statsd.flushInterval;
    }
...
function flushMetrics() {
    ...
    if (old_timestamp > 0) {
        stats.timestamp_lag_ms = (time_stamp - old_timestamp) - settings.statsd.flushInterval;
    }
    ...
    setTimeout(flushMetrics, getFlushTimeout(settings.statsd.flushInterval));
}
...
function handlePacket(msg) { // eslint-disable-line complexity
    ...
    for (const midx in metrics) {
        ...
        if (settings.statsd.dumpMessages) {
...
circonus.init(backendEvents, (err) => {
    if (err !== null) {
        log.error({ err }, 'backend initialization failed, disabling statsd');
        return;
    }

    // Setup the flush timer
    setTimeout(flushMetrics, settings.statsd.flushInterval);
    // start the listener(s)
    for (let i = 0; i < settings.statsd.servers.length; i++) {
        startServer(settings.statsd.servers[i], handlePacket);
    }

    log.info(`${settings.statsd.app_name} listener up`);
});
