...
const log = settings.logger.child({ module: 'statsd' });
...
function getFlushTimeout(interval) {
    let timer_interval = interval - ((Date.now() - settings.start_time) % settings.statsd.config.flushInterval);

    if (timer_interval < 0) {
        log.warn(`calculated negative flush timer_interval (${timer_interval}), resetting to ${settings.statsd.config.flushInterval}...`);
        timer_interval = settings.statsd.config.flushInterval;
    }
...
function flushMetrics() {
    ...
    if (old_timestamp > 0) {
        stats.timestamp_lag_ms = (time_stamp - old_timestamp) - settings.statsd.config.flushInterval;
    }
    ...
    setTimeout(flushMetrics, getFlushTimeout(settings.statsd.config.flushInterval));
}
...
function handlePacket(msg) { // eslint-disable-line complexity
    ...
    for (const midx in metrics) {
        ...
        if (settings.statsd.config.dumpMessages) {
...
setTimeout(flushMetrics, settings.statsd.config.flushInterval);

// load circonus metrics transmitter
circonus.init(backendEvents);

// start the listener(s)
for (let i = 0; i < settings.statsd.config.servers.length; i++) {
    startServer(settings.statsd.config.servers[i], handlePacket);
}
