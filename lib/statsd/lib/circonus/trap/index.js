...
module.exports = class Trap {
    ...
    constructor(check_type) {
        ...
        this.log = settings.logger.child({ submodule: 'circonus-backend-trap', type: `trap.${this.check_type}` });
    ...
    initialize() {
        ...
        return new Promise((resolve, reject) => {
            if (self.check_id === null) {
                reject(new Error(`no ${self.check_type} check available`));
                return;
            }

            if (!self.manage_check_metrics && self.submission_url !== null) {
                self.log.debug(`initialize '${self.check_type}' trap`);
                self.enabled = true;
            ...
            broker.loadCA().
                ...
                then(() => {
                    this.log.debug(`initialize '${this.check_type}' trap`);
                    self.enabled = true;
                    resolve(self.enabled);
                }).
                catch((err) => {
    ...
    _getSubmissionURL() {
        ...
        if (this.check === null) {
            this.log.fatal({ check_type: this.check_type }, 'check is null');
            process.exit(1);
        }
    ...
    _getcheck_cid() {
        if (this.check_cid !== null) {
            return this.check_cid;
        }
