...
function inventoryMetrics(metrics) {
    ...
    for (let i = 0; i < metrics.length; i++) {
        inventory[metrics[i].name] = (/^active$/i).test(metrics[i].status);
    }
...
module.exports = class Trap {
    ...
    constructor(checkType) {
        if (checkType === 'group') {
            ...
            throw new Error(`invalid check type id '${checkType}' passed to Trap constructor`);
        }
        ...
        this.manageMetrics = settings.statsd.manageCheckMetrics;
        this.forceMetricActivation = settings.statsd.forceMetricActivation;

        this.enabled = false;
        this.log = settings.logger.child({ submodule: 'circonus-backend-trap', type: `trap.${this.checkType}` });
    ...
    submit(metrics) {
        ...
        return new Promise((resolve, reject) => {
            if (!this.enabled) {
                reject(new Error(`'${this.checkType}' trap is not enabled`));
                return;
            }
    ...
    _enableMetrics(metrics) {
        ...
        return new Promise((resolve, reject) => {
            ...
            self._loadCheck().
                ...
                then((bundle) => {
                    self.log.trace('new metrics activated, saving updated check definition');
    ...
    _getSubmissionURL() {
        ...
        if (this.check === null) {
            this.log.fatal({ check_type: this.checkType }, 'check is null');
            process.exit(1);
        }
    ...
    _getCheckCID() {
        ...
        this.log.trace(`getting check id from ${this.regFile}`);
    ...
    _saveCheckBundle(bundle) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (self.check !== null && bundle._last_modified === self.check._last_modified) {
                self.log.trace(`check un-modified, skipping write`);
                resolve(bundle);
                return;
            }
            ...
            self.log.trace(`saving up-to-date check definition ${self.regFile}`);
    ...
    _loadCheck() {
        ...
        return new Promise((resolve, reject) => {
            ...
            self.log.trace(`fetching up-to-date check definition from API`);
