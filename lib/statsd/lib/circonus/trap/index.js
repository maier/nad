...
module.exports = class Trap {
    ...
    constructor(check_type) {
        ...
        this.log = log.child({ submodule: 'trap', type: this.check_type });
    ...
    initialize() {
        ...
        return new Promise((resolve) => {
            if (self.check_id === null) {
                self.log.warn({ err: 'no check id supplied' }, 'trap.initialize');
                resolve(false);
                return;
            }

            if (!self.manage_check_metrics && self.submission_url !== null) {
                self.log.debug(`initialized trap`);
                self.enabled = true;
            ...
            broker.loadCA().
                ...
                then(() => {
                    this.log.debug(`initialized trap`);
                    self.enabled = true;
                    resolve(self.enabled);
                }).
                catch((err) => {
