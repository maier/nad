...
function fetchCheckBundle(cid) {
    ...
    return new Promise((resolve, reject) => {
        ...
        client.get(cid, null, (err, body, code) => { // eslint-disable-line consistent-return
            if (err !== null) {
...
function updateCheckBundle(bundle) {
    ...
    return new Promise((resolve, reject) => {
        ...
        client.put(cid, bundle, (err, body, code) => { // eslint-disable-line consistent-return
            if (err !== null) {
                reject(new Error(`${errPrefix} ${err}`));
                return;
            }
...
module.exports = class Trap {
    ...
    _enableMetrics(metrics) {
        ...
        return new Promise((resolve, reject) => {
            ...
            log.debug('checking for new metrics');
            ...
            self._loadCheck().
                then(() => {
                    ...
                    log.debug('activating metrics with API');
                    return updateCheckBundle(check);
                }).
                then((bundle) => {
                    log.debug('new metrics activated, saving updated check definition');
                    return self._saveCheckBundle(bundle);
                }).
                then((bundle) => {
                    self.check = JSON.parse(JSON.stringify(bundle));
                    log.debug('update metric inventory');
    ...
    _sendMetrics(metrics) {
        ...
        return new Promise((resolve, reject) => {
            ...
            log.debug({ metrics }, 'metrics to send');
            ...
            const timeout = setTimeout(() => {
                log.debug('request timeout, calling req abort');
                req.abort();
            }, timeout_ms);
            ...
            req.once('timeout', () => {
                clearTimeout(timeout);
                log.debug('request timeout, abort');
                req.abort();
            });

            req.once('error', (err) => {
                clearTimeout(timeout);
                log.error({ metrics: metricJson }, `sending metrics for ${self.id}`);
                reject(err);
            });

            log.debug({ metrics: metricJson }, 'submitting');
    ...
    _saveCheckBundle(bundle) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (self.check !== null && bundle._last_modified === self.check._last_modified) {
                log.debug(`check un-modified, skipping write`);
                resolve(bundle);
                return;
            }
            ...
            log.debug(`saving up-to-date check definition ${self.regFile}`);
    ...
    _loadCheck() {
        ...
        return new Promise((resolve, reject) => {
            ...
            log.debug(`fetching up-to-date check definition from API`);
    ...
    _loadBrokerCN() { // eslint-disable-line consistent-return
        ...
        return new Promise((resolve, reject) => {
            ...
            log.debug('fetching broker definition with API');
            self.brokerCN = null;
                ...
                then((_broker) => {
                    ...
                    for (const detail of _broker._details) {
                        if (submissionURL.indexOf(detail.ipaddress) !== -1) {
                            log.debug(`setting broker cn to '${detail.cn}'`);
