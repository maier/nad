...
function getMetricDest(name) {
    if (settings.statsd.groupKey === null && settings.statsd.hostKey === null) {
        return 'host';
    }

    if (settings.statsd.groupKey !== null && name.substr(0, settings.statsd.groupKey.length) === settings.statsd.groupKey) {
        return 'group';
    }

    if (settings.statsd.hostKey !== null && name.substr(0, settings.statsd.hostKey.length) === settings.statsd.hostKey) {
        return 'host';
    }
...
function fixMetricName(name, dest) {
    if (dest === 'group' && name.substr(0, settings.statsd.groupKey.length) === settings.statsd.groupKey) {
        return name.substr(settings.stasd.groupKey.length);
    }

    if (dest === 'host' && name.substr(0, settings.statsd.hostKey.length) === settings.statsd.hostKey) {
        return name.substr(settings.stasd.hostKey.length);
    }
...
module.exports = class Circonus {
    ...
    flushMetrics(calc_start, metrics_to_flush) {
        ...
        for (const key in counters) {
            ...
            metricName = fixMetricName(metricName, metricDest);
