...
function getMetricDest(name) {
    ...
    if (settings.statsd.group_key !== null && name.substr(0, settings.statsd.group_key.length) === settings.statsd.group_key) {
        return 'group';
    }

    if (settings.statsd.host_key !== null && name.substr(0, settings.statsd.host_key.length) === settings.statsd.host_key) {
        return 'host';
    }
...
function fixMetricName(name, dest) {
    if (dest === 'group' && name.substr(0, settings.statsd.group_key.length) === settings.statsd.group_key) {
        return name.substr(settings.stasd.group_key.length);
    }

    if (dest === 'host' && name.substr(0, settings.statsd.host_key.length) === settings.statsd.host_key) {
        return name.substr(settings.stasd.host_key.length);
    }
