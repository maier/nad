...
const log = settings.logger.child({ module: 'statsd', submodule: 'circonus-backend' });
...
class Circonus {
    ...
    constructor(events) {
        ...
        if (settings.statsd.config.prefix.global !== '') {
            for (const key in settings.statsd.config.prefix) {
                if (key !== 'global') {
                    this.prefix[key] = [ settings.statsd.config.prefix.global ];
                }
            }
        }
        ...
        for (const key in settings.statsd.config.prefix) {
            if (key !== 'global') {
                ...
                if (settings.statsd.config.prefix[key] !== '') {
                    this.prefix[key].push(settings.statsd.config.prefix[key]);
                }
            }
        }
        ...
        if (settings.statsd.config.suffix.global !== '') {
            for (const key in settings.statsd.config.suffix) {
                if (key !== 'global') {
                    this.suffix[key] = [ settings.statsd.config.suffix.global ];
                }
            }
        }
        ...
        for (const key in settings.statsd.config.suffix) {
            if (key !== 'global') {
                ...
                if (settings.statsd.config.suffix[key] !== '') {
                    this.suffix[key].push(settings.statsd.config.suffix[key]);
                }
            }
        }
    ...
    _initCheck(checkType) {
        ...
        return new Promise((resolve) => {
            self.checks[checkType] = new Trap(checkType, settings.statsd.config.forceMetricActivation);
    ...
    _initializeChecks() {
        ...
        this._initCheck('group').
            then((err) => {
                if (err !== null) {
                    log.error({ err }, 'Unable to load group check');
                    numErrors += 1;
                }
            ...
            then((err) => {
                if (err !== null) {
                    log.error({ err }, 'Unable to load host check');
                    numErrors += 1;
                }
            }).
            then(() => {
                if (numErrors === 2) {
                    log.warn(`No 'host' or 'group' checks found, ${settings.app_name} disabled`);
                    return;
                }

                self.eventManager.on('flush', self.flushMetrics.bind(self));
                log.info(`${settings.app_name} v${settings.app_version} loaded`);
            }).
            catch((err) => {
                log.error(err);
            });
    ...
    flushMetrics(calc_start, metrics) { // eslint-disable-line complexity, max-statements
        ...
        if (settings.statsd.config.forceGC && global.gc) {
            log.debug('force gc');
            global.gc();
        }
        ...
        if (settings.statsd.config.sendProcessStats) {
