...
function setStatsdOptions(options) {
    ...
    instance.statsd = {
        enabled: true,
        logger: instance.logger.child({ module: `${instance.app_name}-statsd` }),
        start_time: Date.now(),
    ...
    try {
        ...
        instance.logger.warn({ err: err.message }, 'unable to find cosi installation, all metrics will be sent to NAD');
    }
    ...
    if (options.statsd_config && options.statsd_config !== null) {
        try {
            ...
            if (err.code !== 'MODULE_NOT_FOUND') {
                instance.logger.fatal({ err: err.message, config_file: options.statsd }, 'unable to load statsd config');
    ...
    if (cfg.servers && Array.isArray(cfg.servers)) {
        for (const server of cfg.servers) {
            if (!server.server || !(/^(udp|tcp)$/).test(server.server)) {
                instance.logger.warn({ server }, `invalid server config [protocol], ignoring`);
                continue;
            }
            if (!server.address || !(net.isIPv4(server.address) || net.isIPv6(server.address))) {
                instance.logger.warn({ server }, `invalid server config [address], ignoring`);
                continue;
            }
            if (!server.port || !(/^\d+$/).test(`${server.port}`)) {
                instance.logger.warn({ server }, `invalid server config [port], ignoring`);
                continue;
            }
    ...
    //      all metrics prefixed with `host.`
    //      all metrics if no statsd_config supplied
    // push_receiver metric category name
    const pr_mc = instance.statsd.prefix.global === '' ? 'host' : instance.statsd.prefix.global;

    instance.statsd.push_receiver_url = `http://${instance.listen[0].address}:${instance.listen[0].port}/write/${pr_mc}`;

    if (instance.statsd.flushInterval < default_flush_interval) {
        instance.logger.warn({ flushInterval: instance.statsd.flushInterval }, `invalid flush interval, using ${default_flush_interval}`);
...
class Settings {
    constructor() {
        ...
        options.
            ...
            option('--broker_ca <file>', `CA file for broker reverse connection []`).
            //
            ...
            //
            // statsd
            option('--statsd', `Enable builtin StatsD interface [false]`).
