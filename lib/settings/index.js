...
function configure_circonus() {
    ...
    try {
        nadapi = require(path.join(nad.lib_dir, 'nad_self_configure'));
    } catch (err) {
...
function setStatsdOptions(options) {
    if (options.statsd) {
        const cosi_dir = path.join('opt', 'circonus', 'cosi');

        try {
            instance.statsd.cosi_config_file = fs.realpathSync(path.join(cosi_dir, 'etc', 'cosi.json'));
            instance.statsd.cosi_system_check_file = fs.realpathSync(path.join(cosi_dir, 'registration', 'registration-check-system.json'));
            instance.statsd.cosi_statsd_check_file = fs.realpathSync(path.join(cosi_dir, 'registration', 'registration-check-statsd.json'));
        } catch (err) {
            if (err.code === 'ENOENT') {
                instance.log.warn({ err: err.message }, 'unable to verify cosi installation, disabling statsd');
                return;
            }
            instance.log.fatal({ err: err.message }, 'unable to access cosi installation');
            console.error(instance.pfx_error, 'unable to access cosi installation', err);
            process.exit(1);
        }
        ...
        try {
            const cfgFile = fs.realpathSync(options.statsd);

            cfg = require(cfgFile);
        } catch (err) {
            instance.log.fatal({ err: err.message, config_file: options.statsd }, 'unable to load statsd config');
            console.error(instance.pfx_error, 'unable to load statsd config', err);
            process.exit(1);
        }

        instance.statsd = {
            logger: instance.log.child({ module: `${instance.app_name}-statsd` }),
            start_time: Date.now(),
        ...
        if (cfg.servers && Array.isArray(cfg.servers)) {
            for (const server of cfg.servers) {
                if (!server.server || !(/^(udp|tcp)$/).test(server.server)) {
                    instance.log.warn({ server }, `invalid server config [protocol], ignoring`);
                    continue;
                }
                if (!server.address || !(net.isIPv4(server.address) || net.isIPv6(server.address))) {
                    instance.log.warn({ server }, `invalid server config [address], ignoring`);
                    continue;
                }
                if (!server.port || !(/^\d+$/).test(`${server.port}`)) {
                    instance.log.warn({ server }, `invalid server config [port], ignoring`);
                    continue;
                }
        ...
        for (const defaultSetting of defaults) {
            ...
            // backfill prefixes/postfixes with defaults
            if (defaultSetting.name === 'prefix' || defaultSetting.name === 'postfix') {
                for (const key of Object.keys(defaultSetting.default)) {
                    if (!{}.hasOwnProperty.call(instance.statsd[defaultSetting.name], key)) {
                        instance.statsd[defaultSetting.name][key] = defaultSetting.default[key];
                    }
                }
            }
        }
        ...
        if (instance.statsd.flushInterval < 10000) {
            instance.log.warn({ flushInterval: instance.statsd.flushInterval }, `invalid flush interval, using 60000.`);
            instance.statsd.flushInterval = 60000;
        }

        instance.statsd.enabled = true;
    }
}
...
class Settings {
    constructor() {
        ...
        options.
            ...
            option('--ssl_cert <file>', `SSL certificate PEM file, required for SSL [${DEFAULT_SSL_CERT}]`, null).
            ...
            option('-c <dir>', `${chalk.yellow('DEPRECATED')} use ${chalk.bold('--plugin_dir')}`, null).
            option('--authtoken <token>', `${chalk.yellow('DEPRECATED')} use ${chalk.bold('--api_key')}`, null).
            option('--apihost <host>', `${chalk.yellow('DEPRECATED')} use ${chalk.bold('--api_url')}`, null).
            option('--apiport <port>', `${chalk.yellow('DEPRECATED')} ${chalk.bold('--api_url')}`, null).
            option('--apipath <path>', `${chalk.yellow('DEPRECATED')} ${chalk.bold('--api_url')}`, null).
            option('--apiprotocol <proto>', `${chalk.yellow('DEPRECATED')} ${chalk.bold('--api_url')}`, null).
            option('--apiverbose', `${chalk.yellow('DEPRECATED')} NOP, see ${chalk.bold('--debug')}`, null).
            option('--sslcert <file>', `${chalk.yellow('DEPRECATED')} use ${chalk.bold('--ssl_cert')}`, null).
            option('--sslkey <file>', `${chalk.yellow('DEPRECATED')} use ${chalk.bold('--ssl_key')}`, null).
            option('--sslca <file>', `${chalk.yellow('DEPRECATED')} use ${chalk.bold('--ssl_ca')}`, null).
            option('--cafile <file>', `${chalk.yellow('DEPRECATED')} use ${chalk.bold('--broker_ca')}`, null).
            on('--help', helpDetails).
            parse(process.argv);
