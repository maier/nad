...
function setPluginDirectory(options) {
    if (options.c) {
        ...
        try {
            ...
            const errMsg = 'invalid plugin dir';

            log.fatal({ dir: options.c, err }, errMsg);
            configError(`${errMsg} '${options.c}' - ${err}`);
        }
    }
    if (options.pluginDir) {
        try {
            instance.plugin_dir = fs.realpathSync(options.pluginDir);
        } catch (err) {
            const msg = 'invalid plugin dir';

            log.fatal({ dir: options.pluginDir, err }, msg);
            configError(`${msg} '${options.pluginDir}' - ${err}`);
        }
    }
}
...
function setStatsdOptions(options) {
    ...
    if (options.statsdConfig && options.statsdConfig !== null) {
        try {
    ...
    const defaults = [
        ...
        {
            name: 'groupCheckBundleID', // use an explicit check bundle for the group check
            active: true, // is this option active
            default: 'group.'
        },
        {
            name: 'groupKey', // **FIRST** part of the metric name that indicates this is a 'group' metric (the groupKey is removed from the metric name)
            active: true, // is this option active
            default: 'group.'
        },
        {
    ...
    if (typeof instance.statsd.groupKey === 'undefined') {
        instance.statsd.groupKey = null;
    }
    if (instance.statsd.groupKey && instance.statsd.groupKey === '') {
        instance.statsd.groupKey = null;
    }

    if (typeof instance.statsd.hostKey === 'undefined') {
        instance.statsd.hostKey = null;
    }
    if (instance.statsd.hostKey && instance.statsd.hostKey === '') {
        instance.statsd.hostKey = null;
    }
