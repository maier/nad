...
function getNoitCreds(cafile) {
    if (cafile) {
        return noit.hashToCreds({ ca: cafile });
    }
...
function setup_reverse(options, logger) {
    ...
    circapi.setup(options.auth_token, 'nad', options.api_options);
    ...
    const noitCreds = getNoitCreds(options.cafile);
    ...
    circapi.get(reqPath, null, (code, err, data) => {
        ...
        for (const rcURL of check._reverse_connection_urls) {
            ...
            revs[rcURL] = new noit.connection(parts[2], parts[1], noitCreds); // eslint-disable-line new-cap
