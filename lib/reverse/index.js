...
function getNoitCreds(cafile, log) {
    if (cafile) {
        try {
            ...
            log.fatal({ err, cafile }, 'unable to load cafile');
            process.exit(1);
        }
    }
...
function setup_reverse(options, logger) {
    ...
    const log = logger.child({ module: 'reverse' });
    ...
    const noitCreds = getNoitCreds(options.cafile, log);
    ...
    circapi.get(reqPath, null, (code, err, data) => {
        ...
        if (data === null || typeof data !== 'object' || Array.isArray(data) && data.length === 0) {
            if (options.check_bundle_id === null) {
                log.fatal({ host: options.hostname }, 'configuration for reverse - searching yielded no applicable json:nad check');
            } else {
                log.fatal({ check_bundle_id: options.check_bundle_id }, 'configuration for reverse - unable to retrieve check object with API.');
            }
            process.exit(1);
        }
        ...
        for (const rcURL of check._reverse_connection_urls) {
            ...
            revs[rcURL] = new noit.Connection(parts[2], parts[1], noitCreds, null, log); // eslint-disable-line new-cap
